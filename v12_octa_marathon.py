...
def run_octa_marathon(audit_only=False):
    marathon_start_time = time.time()
    device = "cuda" if torch.cuda.is_available() else "cpu"
    langs = ["en", "de", "ru", "es", "it", "mr", "ro", "fr"]
    lang_names = {
        "en": "English", "de": "German", "ro": "Romanian", "es": "Spanish",
        "it": "Italian", "ru": "Russian", "mr": "Marathi", "fr": "French"
    }
    n_per_lang = 256
    base_dim = n_per_lang
    ...
    # --- FINAL OCTA-AUDIT ---
    print("\n>>> FINAL OCTA-AUDIT: ZERO-FORGETTING VALIDATION (GATED ISOLATION) <<<")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--audit_only":
        run_octa_marathon(audit_only=True)
    else:
        run_octa_marathon()
