...
class EnhancedAGNISV2:
    def __init__(
        ...
        use_tensorized_forward: bool = False,
        tensor_graph_cache_enabled: bool = True,
        output_head_enabled: bool = True,
        output_head_lr: float = 0.05,
        self_supervised_enabled: bool = True,
        self_supervised_noise: float = 0.05,
        plasticity_decay: float = 0.01,
        plasticity_recovery: float = 0.02,
        plasticity_min: float = 0.2,
        drift_window: int = 50,
        autonomous_replay_boost: float = 0.5,
        ...
        self.use_tensorized_forward = use_tensorized_forward
        self.tensor_graph_cache_enabled = tensor_graph_cache_enabled
        self._tensor_graph_cache: Dict[Tuple[bool, Optional[int]], Dict[str, torch.Tensor]] = {}
        self._tensor_graph_version = 0
        self.output_head_enabled = output_head_enabled
        self.output_head_lr = max(1e-4, output_head_lr)
        self.output_head_W = torch.randn(self.output_dim, self.output_dim) * 0.01
        self.output_head_b = torch.zeros(self.output_dim)
        self.self_supervised_enabled = self_supervised_enabled
        self.self_supervised_noise = max(0.0, self_supervised_noise)
        self.plasticity_budget = 1.0
        self.plasticity_decay = max(0.0, plasticity_decay)
        self.plasticity_recovery = max(0.0, plasticity_recovery)
        self.plasticity_min = max(0.0, min(1.0, plasticity_min))
        self.drift_history = deque(maxlen=max(10, drift_window))
        self.autonomous_replay_boost = max(0.0, autonomous_replay_boost)
        ...
        self.learning_first_mode = False
    ...
    def _invalidate_tensor_cache(self):
        if not self.tensor_graph_cache_enabled:
            return
        self._tensor_graph_version += 1
        self._tensor_graph_cache.clear()
    ...
    def begin_task(self, task_id: int):
        ...
        if self.learning_first_mode:
            self._apply_learning_first_preset()
    ...
    def enable_learning_first(self, enabled: bool = True):
        self.learning_first_mode = enabled
        if enabled:
            self._apply_learning_first_preset()

    def _apply_learning_first_preset(self):
        # Balanced preset: faster learning without full instability.
        self.hebb_noise_scale = max(0.05, min(0.12, self.hebb_noise_scale * 1.5))
        self.hebb_corr_ema = max(0.6, min(0.9, self.hebb_corr_ema * 0.8))
        self.hebb_corr_clip = max(0.7, min(1.2, self.hebb_corr_clip * 1.1))
        self.hebb_active_threshold = max(0.005, self.hebb_active_threshold * 0.7)
        self.mastery_lr_multiplier = max(self.mastery_lr_multiplier, 6.0)
        self.mastery_corr_ema_scale = min(self.mastery_corr_ema_scale, 0.7)
        self.mastery_forward_steps = max(self.mastery_forward_steps, 3)
        self.freeze_base_after_tasks = max(self.freeze_base_after_tasks, 50)
        self.intrinsic_engine.ema = max(0.9, min(0.98, self.intrinsic_engine.ema * 0.95))
        self.salience_engine.ema = max(0.9, min(0.98, self.salience_engine.ema * 0.95))

    def _forward_features(self, x: torch.Tensor, num_steps: int = 2) -> torch.Tensor:
        if x.dim() == 1:
            x = x.unsqueeze(0)
        ...
        feats = torch.stack([self.neurons[nid].activation.mean() for nid in self.output_neurons])
        if batch_size > 1:
            feats = feats.unsqueeze(0).expand(batch_size, -1)
        return feats

    def _apply_output_head(self, feats: torch.Tensor) -> torch.Tensor:
        if feats.dim() == 1:
            feats = feats.unsqueeze(0)
        if feats.device != self.output_head_W.device:
            feats = feats.to(self.output_head_W.device)
        return feats @ self.output_head_W + self.output_head_b

    def _update_plasticity_budget(self, drift_signal: float) -> float:
        self.drift_history.append(float(drift_signal))
        avg_drift = sum(self.drift_history) / max(1, len(self.drift_history))
        self.plasticity_budget = self.plasticity_budget + self.plasticity_recovery - self.plasticity_decay * avg_drift
        self.plasticity_budget = max(self.plasticity_min, min(1.0, self.plasticity_budget))
        return self.plasticity_budget

    def learn_step(self, x: torch.Tensor, y: Optional[torch.Tensor] = None, task_id: Optional[int] = None):
        if y is None and self.self_supervised_enabled:
            noise = self.self_supervised_noise
            if noise > 0:
                x_noised = x + noise * torch.randn_like(x)
            else:
                x_noised = x
            if self.use_tensorized_forward:
                y_pseudo = self.forward_tensorized(x_noised, num_steps=2, active_only=True)
            else:
                y_pseudo = self.forward(x_noised, num_steps=2)
            y = y_pseudo.detach()
        return self.learn(x, y, task_id=task_id)

    def run_stream(self, stream, steps: Optional[int] = None):
    ...
    def _update_output_head(self, feats: torch.Tensor, error_head: torch.Tensor, lr_scale: float = 1.0):
        if not self.output_head_enabled:
            return
        if feats.dim() == 1:
            feats = feats.unsqueeze(0)
        if error_head.dim() == 1:
            error_head = error_head.unsqueeze(0)
        if feats.device != self.output_head_W.device:
            feats = feats.to(self.output_head_W.device)
        if error_head.device != self.output_head_W.device:
            error_head = error_head.to(self.output_head_W.device)
        batch = max(1, feats.shape[0])
        grad_W = feats.t() @ error_head / batch
        grad_b = error_head.mean(dim=0)
        lr = self.output_head_lr * max(0.0, lr_scale)
        self.output_head_W += lr * grad_W
        self.output_head_b += lr * grad_b

    def _compute_output_and_errors(
        self,
        x: torch.Tensor,
        y: torch.Tensor,
        num_steps: int,
        ...
        if use_tensorized:
            feats = self.forward_tensorized(x, num_steps=num_steps, active_only=True, return_features=True)
        else:
            feats = self._forward_features(x, num_steps=num_steps)
        output = self._apply_output_head(feats) if self.output_head_enabled else feats
        if y.dim() == 1:
            y = y.unsqueeze(0)
        if output.dim() == 1:
            output = output.unsqueeze(0)
        error_head = y - output
        if self.output_head_enabled:
            error_out = error_head @ self.output_head_W.t()
        else:
            error_out = error_head
        return output, error_out, feats

    def forward(self, x: torch.Tensor, num_steps: int = 2) -> torch.Tensor:
        feats = self._forward_features(x, num_steps=num_steps)
        if self.output_head_enabled:
            return self._apply_output_head(feats)
        return feats
    ...
    def build_tensor_graph(self, active_only: bool = True) -> Dict[str, torch.Tensor]:
        ...
        """
        if self.tensor_graph_cache_enabled:
            cache_key = (active_only, self.current_task_id)
            cached = self._tensor_graph_cache.get(cache_key)
            if cached and cached.get("version") == self._tensor_graph_version:
                return cached["graph"]
        ...
        if not node_ids:
            graph = {"node_ids": [], "edge_index": torch.empty((2, 0), dtype=torch.long), "edge_weight": torch.empty((0, self.neuron_dim, self.neuron_dim))}
            if self.tensor_graph_cache_enabled:
                self._tensor_graph_cache[cache_key] = {"version": self._tensor_graph_version, "graph": graph}
            return graph
    ...
    def forward_tensorized(self, x: torch.Tensor, num_steps: int = 2, active_only: bool = True, return_features: bool = False) -> torch.Tensor:
        """
        ...
        if batch_size > 1:
            outputs = outputs.unsqueeze(0).expand(batch_size, -1)
        if return_features or not self.output_head_enabled:
            return outputs
        return self._apply_output_head(outputs)
    ...
    def learn(self, x: torch.Tensor, y: torch.Tensor, task_id: Optional[int] = None, error_propagation_steps: int = 1):
        ...
        output, error_out, feats = self._compute_output_and_errors(
            x=x,
            y=y,
            num_steps=forward_steps,
            use_tensorized=self.use_tensorized_forward
        )
        if y.dim() == 1:
            y = y.unsqueeze(0)
        ...
        if self.self_audit_enabled:
            ...
            self.last_audit_signal = 0.0
        if self.autonomous_replay_boost > 0 and self.last_audit_signal > 0:
            mods["replay_scale"] *= (1.0 + self.autonomous_replay_boost * self.last_audit_signal)
        drift_signal = float(loss.item()) + 0.5 * self.last_audit_signal
        lr_scale = self._update_plasticity_budget(drift_signal)
        self._update_output_head(feats, error, lr_scale=lr_scale)
        ...
        for i, nid in enumerate(self.output_neurons):
            if i < error_out.shape[1]:
                error_signals[nid] = error_out[:, i].mean().repeat(self.neuron_dim)

        self._propagate_error(
            error_signals=error_signals,
            task_id=task_id,
            steps=error_propagation_steps,
            error_scale=0.3 * salience_weight * mods["error_scale"] * lr_boost * lr_scale * (1.0 + self.self_pred_loss_weight * self.self_model.self_pred_error),
            usage_threshold=5,
            corr_ema=corr_ema,
            lr_multiplier=lr_boost * lr_scale
        )
    ...
    def _replay_step(self, batch_size: int = 10, distill_weight: float = 0.7, replay_scale: float = 1.0):
        ...
        for tid, x, y, teacher, salience_weight in samples:
            ...
            if self.use_tensorized_forward:
                feats = self.forward_tensorized(x, num_steps=replay_steps, active_only=True, return_features=True)
            else:
                feats = self._forward_features(x, num_steps=replay_steps)
            output = self._apply_output_head(feats) if self.output_head_enabled else feats
            if output.dim() == 1:
                output = output.unsqueeze(0)
            if teacher.dim() == 1:
            ...
            error = y - output
            self._update_output_head(feats, error, lr_scale=self.plasticity_budget)
            if self.output_head_enabled:
                error_out = error @ self.output_head_W.t()
            else:
                error_out = error
            error_signals = {}
            for i, nid in enumerate(self.output_neurons):
        ...
        if loss_value > self.oracle_spike_ratio * avg:
            ...
            for _ in range(self.oracle_replay_burst):
                ...
                for tid, x, y, teacher in samples:
                    self._set_active_task(tid)
                    if self.use_tensorized_forward:
                        feats = self.forward_tensorized(x, num_steps=2, active_only=True, return_features=True)
                    else:
                        feats = self._forward_features(x, num_steps=2)
                    output = self._apply_output_head(feats) if self.output_head_enabled else feats
                    if output.dim() == 1:
                        output = output.unsqueeze(0)
                    if teacher.dim() == 1:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            "tensorized_forward": bool(self.use_tensorized_forward),
            "tensor_graph_cache": bool(self.tensor_graph_cache_enabled)
        }
