...
Paste this completed into every report. An unchecked box is a blocker, not a note.
...
1. **Permanent Control Arm**:
   ...
   *(Cross-reference: Enforced by Section 11.3, Section 11.5; Conflict documented in Section 15.1)*
...
7. **Seed Before Construction (Rule R17)**:
   ...
   *(Cross-reference: Enforced by Section 5.5, Section 9.3; Conflict documented in Section 15.2)*
...
## 14. Compliance Matrix (Rules 1 to 11)

| Rule # | Rule Name | Dual Identifier | Current Enforcement Status | Enforcement Implementation & Gaps |
|:---|:---|:---|:---|:---|
| **Rule 1** | Permanent Control Arm (`FREEZE-AFTER-BASE`) | — | **Conflict / Unenforced** | Mandates `FREEZE-AFTER-BASE` in every table, but the arm was withdrawn due to BatchNorm leakage. No CI script checks table inclusion. (Conflict 15.1). |
...
| **Rule 7** | Seed Before Construction | Rule R17 | **Conflict / Partial Executable** | `torch.manual_seed()` is enforced at startup; but 5-seed requirement `[42,43,44,45,46]` conflicts with recent 3-seed / single-draw practice (Conflict 15.2). |
...
## 15. Outstanding Rule Conflicts (Awaiting Human Ruling)
...
### 15.1 Conflict A: Rule 1 vs. Withdrawn Control Arm
...
### 15.2 Conflict B: Rule 7 (R17) vs. Experimental Seed Practice
