"""
train_chl_fluency_model.py
==========================
Head-to-head comparison:
Trains a FRESH AGNIS core using CHL / Target Propagation, driven by the error
from the Fusion MLP head, using the exact same massive corpus and setup.

Design:
1. The Fusion MLP head learns via standard Backprop (AdamW).
2. The error at the core interface is intercepted as `core_leaf.grad`.
3. This error generates a Target Hidden State.
4. The biological AGNIS core uses `infer_and_learn` (SNAP-ATP) to morph 
   towards the Target Hidden State.
"""
...
MODEL_OUT = "agnis_chl_fluency_model_en.pt"
...
EPOCHS = 20
LR_HEAD = 4e-4
LR_CORE_TP = 1.0  # Learning rate scaling for Target Propagation error
WARMUP_STEPS = 1000
LOG_EVERY = 500
MAX_GEN_TOKENS = 80
TEMPERATURE = 0.8
...
def main() -> None:
    ...
    # NOTE: We DO NOT load the marathon checkpoint!
    # NOTE: We DO NOT freeze the core!
    ...
    n_params_head = sum(p.numel() for p in trainable)
    n_params_core = sum(p.numel() for p in model.hierarchy.parameters())
    ...
    optimizer = torch.optim.AdamW(trainable, lr=LR_HEAD, weight_decay=0.01)
    best_val = float("inf")
    best_epoch = 0
    stale_epochs = 0
    ...
    for epoch in range(EPOCHS):
        ...
        for step in range(train_tokens.shape[1] - 1):
            ...
            # 1. Forward Pass (Intercepting Core Prediction)
            ...
            # Create a leaf node to intercept gradients from the LM Head
            ...
            # 3. Target Propagation for the Biological Core (CHL)
            if core_leaf.grad is not None:
                # The explicit mathematical error projected backward from the LM head
                grad_core = core_leaf.grad
                
                # We tell the core what its raw hidden state *should* have been
                target_hid = core_pred_trunc - (LR_CORE_TP * grad_core)
                
                # Pad if necessary to match the full 1024 hierarchy dim
                if core_pred_raw.shape[1] > model.embed_dim:
                ...
                # Morph the core toward the target using explicit local Hebbian updates
                model.hierarchy.infer_and_learn(emb.detach(), top_level_label=target_full, max_steps=4)
        ...
        if improved:
            ...
            model.hierarchy.save_checkpoint("agnis_chl_core.pt")
            print("  [Saved best checkpoints: Head + Core]")
        else:
            stale_epochs += 1
    ...
    model.hierarchy.load_checkpoint("agnis_chl_core.pt")
    model._tokenizer = tokenizer
    print(f"[Best] Epoch {best_epoch} | Val Loss {best_val:.4f} | Val PPL {math.exp(min(best_val, 20)):.1f}")
    
    for prompt in PROMPTS:
        out = model.generate(
            prompt, max_new_tokens=MAX_GEN_TOKENS, temperature=TEMPERATURE, top_k=TOP_K, repetition_penalty=REPETITION_PENALTY
        )
        safe = out.encode("ascii", errors="replace").decode("ascii")
        print(f"\nPrompt: {prompt}\nOutput: {safe}")
